...
- 🏗️ `groove-utilities-memory-install` — scaffolds `mistakes.md`, `promises.md`, and `learned/` on install.
...
- ⚙️ `groove-admin-config` — always writes `specs:` and `groovebook:` keys explicitly — no absent-means-default ambiguity.
