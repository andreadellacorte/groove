...
- Read `.groove/index.md` frontmatter to determine `tasks:` backend and `task.list_limit` (optional; default 15)
- If `tasks: none`, print a friendly no-op message and exit
- Backend mappings:
  - `beans`: run `skills/groove-utilities-task-list/scripts/list-tasks-by-priority.sh [LIMIT]` with LIMIT from `task.list_limit` (or 15), then parse and format output (group by status: in-progress, todo, blocked)
