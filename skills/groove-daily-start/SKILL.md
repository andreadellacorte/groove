...
- Start task created in configured backend (if `tasks != none`) with task summary and focus suggestion in the body
...
- Create start task via `/groove-utilities-task-create` if `tasks != none`:
  - Title: `YYYY-MM-DD Start`
  - Body: include the task summary from task-analyse (in-progress items, ready/high-priority items) and the suggested focus (top 2–3 priorities) so the bean is the single durable record of "what's ready and what to focus on today", not just a pointer to the daily file
