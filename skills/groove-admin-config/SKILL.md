...
| `specs` | `""` (use default) | any path or blank | "Custom specs directory? Leave blank to use the default (<memory>/specs/). Set to e.g. `specs/` to store specs at the project root for team visibility." |
...
| `groovebook` | `""` (disabled) | `<owner>/<repo>` or blank | "Groovebook repo for sharing workflow learnings? (e.g. `andreadellacorte/groovebook`). Leave blank to keep disabled." |
...
- If the user leaves `specs:` blank, write `specs: ""` — empty string means default (`<memory>/specs/`)
- If the user leaves `groovebook:` blank, write `groovebook: ""` — empty string means disabled
